...
export function SiteHeader({ lang, onToggle, solid = false }: Props) {
  ...
  return (
    <header className={`site-header ${solid ? 'solid' : ''}`}>
      <a className="brand brand-official" href={sitePath('/')} aria-label="HortiVita"><img src={sitePath('/brand-logo.webp')} alt="HortiVita" /></a>
      <nav className={open ? 'nav open' : 'nav'} aria-label={lang === 'pt' ? 'Principal' : 'Main'}>
        {labels.map((item, i) => <a key={item} href={sitePath(links[i])} onClick={() => setOpen(false)}>{item}</a>)}
      </nav>
      <div className="header-actions">
        ...
        <a className="contact-pill" href={sitePath('/contato')}>{lang === 'pt' ? 'Fale conosco' : 'Talk to us'}<ArrowUpRight size={16} /></a>
